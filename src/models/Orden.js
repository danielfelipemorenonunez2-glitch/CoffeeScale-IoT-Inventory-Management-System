...
const Orden = {
  async getAll() {
    if (useMock()) {
      try { return await require('../config/mockapi').getOrdenes(); } catch {}
    }
    return read();
  },

  async crear(deviceId, tiendaId, pesoActual) {
    const nueva = { ordenId: uuidv4(), deviceId, tiendaId, pesoActual, estado: 'PENDIENTE', intentos: 0, fechaCreacion: new Date().toISOString() };
    if (useMock()) {
      try { return await require('../config/mockapi').crearOrden(nueva); } catch {}
    }
